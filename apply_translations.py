...
for lang in languages:
    ...
    existing_keys = re.findall(r"\$string\['(.*?)'\]\s*=", content)
    ...
    for key, val in lang_strings.items():
        if key not in existing_keys:
            ...
            new_lines.append(f"$string['{key}'] = '{escaped_val}';\n")
